...
type CardContent = React.ReactNode | ((sendToBack: () => void) => React.ReactNode);
...
export default function Stack({
  ...
  const [stack, setStack] = useState<{ id: number; content: CardContent }[]>(() => {
    if (cards.length) {
  ...
  return (
    <div className="stack-container" onMouseEnter={() => pauseOnHover && setIsPaused(true)} onMouseLeave={() => pauseOnHover && setIsPaused(false)}>
      {stack.map((card, index) => {
        ...
        const contentToRender = typeof card.content === 'function' 
            ? card.content(() => sendToBack(card.id)) 
            : card.content;
